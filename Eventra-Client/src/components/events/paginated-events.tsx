...
import { FEATURED_EVENTS, DEFAULT_QUERY_LIMIT, type EventItem } from "@/lib/constants";
import { Format } from "@/lib/format";

/**
 * Builds a comprehensive mock dataset from the constants file so there is
 * ONE source of truth. If you update FEATURED_EVENTS in lib/constants.ts,
 * this fallback automatically reflects those changes.
 */
...
const fetchEvents = async (page: number): Promise<PaginatedResponse> => {
  try {
    const res = await api.get(`/events?page=${page}&limit=${DEFAULT_QUERY_LIMIT}`);
    return res.body as unknown as PaginatedResponse;
  } catch {
    // Graceful fallback — paginate from the single source of truth in constants
    const { expanded, totalPages } = buildMockPages(FEATURED_EVENTS, DEFAULT_QUERY_LIMIT);
    ...
    <section className="mb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-8">
        ...
        <div className="flex items-center gap-2 mt-4 md:mt-0">
          ...
          <span className="text-xs font-bold text-slate-500 dark:text-slate-400 tabular-nums min-w-[4.5rem] text-center">
          ...
          {events.map((event, idx) => (
            ...
            >
              ...
              <div className="relative sm:w-48 sm:min-w-48 aspect-video sm:aspect-auto overflow-hidden bg-slate-100 dark:bg-slate-950 shrink-0">
                ...
                {event.badge && (
                  <span className="absolute top-3 left-3 px-2 py-0.5 text-[9px] font-extrabold tracking-widest rounded-full uppercase bg-emerald-500 text-slate-950 shadow">
                    {event.badge}
                  </span>
                )}
              </div>
